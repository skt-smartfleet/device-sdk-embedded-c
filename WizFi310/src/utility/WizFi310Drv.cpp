...
char rcvd_mqtt_topic[128]; //tested by tom 17.09.14
...
void WizFi310Drv::stopClient(uint8_t sock)
{
    ...
    // Check socket
    if (SendCmdWithTag("AT+SMGMT=?\r", "Number of Sockets : 0 (SCID/Mode/Remote/Local/DataMode)\r", "", 1000) == TAG_OK) {
        _state[sock] = NA_STATE;
...
{
	static int is_mqtt_data = 0;  //tested by tom 17.09.14
	static int count=0; //tested by tom 17.09.14
    switch(m_esc_state)
    {
    case ESC_IDLE:
//        LOGDEBUG("ESC_IDLE");
...
//        LOGDEBUG("ESC_CID");
        ...
        else if( (char)recv_data == ',')
        {
            m_esc_state = ESC_PEERIP;
        }
		else if ( recv_data == 'Q' ) // tested by tom 17.09.14
		{
			count =0;
			m_client_sock = 0;
			is_mqtt_data = 1;
    ...
    case ESC_PEERIP:
//        LOGDEBUG("ESC_PEERIP");
        if((((recv_data >= '0') && (recv_data <= '9')) || (recv_data == '.')) && is_mqtt_data == 0 ) //tested by tom 17.09.14
        {
		...
		else if( is_mqtt_data == 1) //tested by tom 17.09.14
		{
			rcvd_mqtt_topic[count] = recv_data;
			count++;
		}
...
////////////////////////////////////////////////////////////////////////////
// MQTT functions    //tested by tom 17.09.18
////////////////////////////////////////////////////////////////////////////

int WizFi310Drv::mqttSet(const char *username, const char *passphrase, const char *clientid, int alivetime)
{
	if( sendCmd(F("AT+MQTTSET=%s,%s,%s,%d\r"),1000,username, passphrase, clientid, alivetime  ) == TAG_ERROR )                 return false;
	return true;
}

int WizFi310Drv::mqttConnect(IPAddress ip)
{
	if( sendCmd(F("AT+MQTTCON=1,%d.%d.%d.%d,1883,0\r"),1000, ip[0], ip[1], ip[2], ip[3]  ) == TAG_ERROR )                 return false;
	return true;

}

int WizFi310Drv::mqttSub(const char *topic)
{
	if( sendCmd(F("AT+MQTTSUB=1,%s\r"),1000, topic ) == TAG_ERROR )                 return false;
	return true;
}

////////////////////////////////////////////////////////////////////////////
// SKT ThungPlug functions    //tested by tom 17.09.20
////////////////////////////////////////////////////////////////////////////

int WizFi310Drv::connectThingPlug(int keepalive, const char *thingplugID, const char *credentialID, const char *serviceID, const char *deviceID)
{
	char resp_ok[8] = "";
	sprintf_P(resp_ok,PSTR("[OK]\r\n"));
	
	if( SendCmdWithTag(F("AT+SKTPCON=1,mqtt.thingplug.net,1883,%d,%s,%s,%s,v1_0,%s\r"),resp_ok,"",10000,keepalive, thingplugID, credentialID, serviceID, deviceID  ) == TAG_ERROR )                 return false;
	//sendCmd(F("AT+SKTPCON=1,mqtt.thingplug.net,1883,%d,%s,%s,%s,v1_0,%s\r"),30000,keepalive, thingplugID, credentialID, serviceID, deviceID  );
	ringBuf.reset();
	return true;
}

int WizFi310Drv::createDevice(const char *deviceID)
{
	char resp_ok[8] = "";
	sprintf_P(resp_ok,PSTR("[OK]\r\n"));
	
	if( SendCmdWithTag(F("AT+SKTPDEVICE=1,%s,%s\r"),resp_ok,"",2000, deviceID ) == TAG_ERROR )                 return false;
	ringBuf.reset();
	return true;
}
int WizFi310Drv::deleteDevice(const char *deviceID, const char *serviceAE)
{
	if( sendCmd(F("AT+SKTPDEVICE=0,%s,%s\r"),2000,deviceID, serviceAE ) == TAG_ERROR )                 return false;
	return true;
}
int WizFi310Drv::createContainer(const char *contatiner)
{
	int status = 1;
	char resp_ok[8] = "";
	sprintf_P(resp_ok,PSTR("[OK]\r\n"));
	
	if( SendCmdWithTag(F("AT+SKTPCONTAINER=1,%s\r"),resp_ok,"",2000,contatiner ) == TAG_ERROR )                 return false;
	ringBuf.reset();
	return true;
}
int WizFi310Drv::deleteContainer(const char *contatiner)
{
	char resp_ok[8] = "";
	sprintf_P(resp_ok,PSTR("[OK]\r\n"));
	if( SendCmdWithTag(F("AT+SKTPCONTAINER=0,%s\r"),resp_ok,"",2000,contatiner ) == TAG_ERROR )                 return false;
	return true;
}
int WizFi310Drv::send(const char *contatiner, const char *data)
{
	int status = 1;
	char resp_ok[8] = "";
	ringBuf.reset();
	...
	status = SendCmdWithTag(F("AT+SKTPSEND=%s,%s\r"),resp_ok,"",2000,contatiner, data );
...
int WizFi310Drv::createCommand(const char *cmd)
{
	if( sendCmd(F("AT+SKTPCMD=1,%s\r"),2000,cmd ) == TAG_ERROR )                 return false;
	return true;
}
int WizFi310Drv::deleteCommand(const char *cmd)
{
	if( sendCmd(F("AT+SKTPCMD=0,%s\r"),2000,cmd ) == TAG_ERROR )                 return false;
	return true;
}
int WizFi310Drv::commandResult(const char *cmd, int exs, int exr)
{
	if( sendCmd(F("AT+SKTPRESULT=%s,%d,%d\r"),2000,cmd, exs, exr ) == TAG_ERROR )                 return false;
	return true;
}
